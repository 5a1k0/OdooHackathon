...
const signUp = async (req, res, next) => {
    ...
    const { id, first_name, last_name, email, password, age, gender } = req.body;

    const hashPassword = bcryptjs.hashSync(password, 10) //it convert the password into hashcode and make it hard

    pool.query(query.createUser, [id, first_name, last_name, email, hashPassword,age, gender], (err, result) => {
        if (err) {
            console.error(err);
